    ...
    protected Collection<Move> calculateKingCastles(Collection<Move> playerLegals, Collection<Move> opponentsLegal) {
        ...
        if(this.playerKing.isFirstMove() && !this.isInCheck()) {
            ...
            if(!this.board.getTile(1).isTileOccupied() && !this.board.getTile(2).isTileOccupied() &&
                ...
                if(rookTile.isTileOccupied() && rookTile.getPiece().isFirstMove() && Player.calculateAttacksOnTile(2, opponentsLegal).isEmpty() &&
                Player.calculateAttacksOnTile(3, opponentsLegal).isEmpty() && rookTile.getPiece().getPieceType().isRook()) {
